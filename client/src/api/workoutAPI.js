...
export const getNextExercise = async (
  ...
  try {
    ...
    const response = await axiosInstance.post(
      API_ENDPOINTS.NEXT_EXERCISE(userWorkoutId),
      payload,
    );
    ...
    throw (
      error.response?.data || {
        message: "Ошибка получения следующего упражнения",
      }
    );
  }
};
...
export const nextWarmup = async (userWorkoutId, currentWarmupId) => {
  try {
    ...
    const response = await axiosInstance.post(
      API_ENDPOINTS.NEXT_WARMUP(userWorkoutId),
      {
    ...
    throw (
      error.response?.data || { message: "Ошибка получения следующей разминки" }
    );
  }
};
...
export const saveExerciseResult = async (
  ...
  try {
    ...
    const response = await axiosInstance.post(
      API_ENDPOINTS.SAVE_EXERCISE_RESULT(userWorkoutId),
      {
...
export const completeWorkout = async (userWorkoutId) => {
  try {
    ...
    const response = await axiosInstance.post(
      API_ENDPOINTS.COMPLETE_WORKOUT(userWorkoutId),
    );
...
// Досрочно завершить разминку и перейти к первому упражнению основной тренировки
export const completeWarmup = async (userWorkoutId) => {
  try {
    console.log("📤 Completing warmup early:", { userWorkoutId });

    const response = await axiosInstance.post(
      API_ENDPOINTS.COMPLETE_WARMUP(userWorkoutId),
    );

    console.log("✅ Warmup completed:", response.data);
    return response.data;
  } catch (error) {
    console.error("❌ Error completing warmup:", error.response?.data);
    throw error.response?.data || { message: "Ошибка завершения разминки" };
  }
};

export const abandonWorkout = async (userWorkoutId) => {
  try {
    console.log("📤 Abandoning workout:", { userWorkoutId });

    const response = await axiosInstance.post(
      API_ENDPOINTS.ABANDON_WORKOUT(userWorkoutId),
    );

    console.log("✅ Workout abandoned:", response.data);
    return response.data;
  } catch (error) {
    console.error("❌ Error abandoning workout:", error.response?.data);
